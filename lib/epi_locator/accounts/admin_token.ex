defmodule EpiLocator.Accounts.AdminToken do
  @moduledoc """
  ...
  schema "admins_tokens" do
    ...
    belongs_to(:admin, EpiLocator.Accounts.Admin)
  ...
  def build_session_token(admin) do
    ...
    {token, %EpiLocator.Accounts.AdminToken{token: token, context: "session", admin_id: admin.id}}
  end
  ...
  defp build_hashed_token(admin, context, sent_to) do
    ...
    {Base.url_encode64(token, padding: false),
     %EpiLocator.Accounts.AdminToken{
       token: hashed_token,
       context: context,
       sent_to: sent_to,
       admin_id: admin.id
  ...
  def token_and_context_query(token, context) do
    from(EpiLocator.Accounts.AdminToken, where: [token: ^token, context: ^context])
  end
  ...
  def admin_and_contexts_query(admin, :all) do
    from(t in EpiLocator.Accounts.AdminToken, where: t.admin_id == ^admin.id)
  end

  def admin_and_contexts_query(admin, [_ | _] = contexts) do
    from(t in EpiLocator.Accounts.AdminToken, where: t.admin_id == ^admin.id and t.context in ^contexts)
  end
end
